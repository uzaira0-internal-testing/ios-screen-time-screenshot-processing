...
class TestConnectionManagerDisconnect:
    ...
    async def test_disconnect_returns_username(self):
        ...
        ws = _make_ws()
        await mgr.connect(ws, user_id=1, username="alice")
        username = mgr.disconnect(1)
        assert username == "alice"

    def test_disconnect_unknown_user_does_not_crash(self):
        mgr = ConnectionManager()
        result = mgr.disconnect(999)
    ...
    async def test_disconnect_idempotent(self):
        ...
        await mgr.connect(ws, user_id=1, username="alice")
        mgr.disconnect(1)
        mgr.disconnect(1)  # second call should not raise
        assert not mgr.is_user_connected(1)
...
class TestConnectionManagerBroadcast:
    ...
    async def test_broadcast_except_excludes_user(self):
        ...
        event = WebSocketEvent.create("test", {})
        await mgr.broadcast_except(event, exclude_user_id=1)
    ...
    async def test_broadcast_except_disconnects_failed_client(self):
        ...
        event = WebSocketEvent.create("test", {})
        await mgr.broadcast_except(event, exclude_user_id=999)
...
class TestConnectionManagerReplace:
    @pytest.mark.asyncio
    async def test_reconnect_replaces_old_socket(self):
        """Same user_id connecting again should replace the old WebSocket."""
        mgr = ConnectionManager()
        ws1 = _make_ws()
        ws2 = _make_ws()
        await mgr.connect(ws1, user_id=1, username="alice")
        await mgr.connect(ws2, user_id=1, username="alice")
        assert mgr.active_connections[1] is ws2
