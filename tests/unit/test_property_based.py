...
class TestSchemaProperties:
    ...
    @given(bad_minutes=st.floats(min_value=60.01, max_value=1000, allow_nan=False, allow_infinity=False))
    @settings(max_examples=30)
    def test_annotation_rejects_minutes_over_60(self, bad_minutes: float):
        """AnnotationBase rejects per-hour values > 60."""
        from screenshot_processor.web.database.schemas import AnnotationBase
