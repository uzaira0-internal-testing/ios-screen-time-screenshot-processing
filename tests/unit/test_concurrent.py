...
class TestScreenshotLocking:
    ...
    async def test_get_screenshot_for_update_uses_for_update(self):
        ...
        mock_result.scalar_one_or_none.return_value = mock_screenshot

        mock_db = AsyncMock()
        mock_db.execute.return_value = mock_result

        result = await get_screenshot_for_update(mock_db, 1)

        assert result == mock_screenshot
        # Verify execute was called (the actual SQL is an implementation detail)
    ...
    async def test_get_screenshot_for_update_raises_404_when_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_screenshot_for_update(mock_db, 999)
