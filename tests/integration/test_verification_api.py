...
CRITICAL: These tests verify that verified_by_user_ids is ACTUALLY persisted
...
class TestVerifyScreenshot:
    ...
    async def test_verify_is_idempotent(
        ...
        # CRITICAL: Check database directly
        await db_session.refresh(test_screenshot)
...
class TestUnverifyScreenshot:
    ...
    CRITICAL: These tests verify that unverify ACTUALLY removes user from
    ...
    async def test_unverify_last_user_sets_null(
        ...
        # CRITICAL: Check database directly - should be None, not empty list
        await db_session.refresh(test_screenshot)
