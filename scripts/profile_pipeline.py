...
using real screenshots from the TECH study (Dagster pipeline source data).
...
Usage:
    ...
    # Run profiling with TECH study screenshots (default):
    ...
    # Use reference_images instead of TECH study:
...
# Fallback: Raw TECH study screenshot paths (unprocessed)
TECH_STUDY_RAW_SCREENSHOT_DIR = Path("W:/Projects/TECH Study/Data/Screenshot Data/Screenshot Raw Data/Screen Time Screenshots")
...
class PipelineProfiler:
    ...
    def _load_tech_study_images(self) -> None:
        """Load TECH study screenshots from Delta Lake or raw directory.
    ...
    def _load_raw_tech_screenshots(self) -> None:
        """Load raw TECH study screenshots from network drive (fallback).

        Used when Delta table is not available.
        """
        raw_dir = TECH_STUDY_RAW_SCREENSHOT_DIR
        ...
        print(f"Scanning raw TECH study screenshots from: {raw_dir}")
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use reference_images directory instead of TECH study screenshots",
    )
    parser.add_argument(
        "--source-dir",
        type=str,
