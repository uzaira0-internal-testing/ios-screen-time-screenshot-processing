...
def _get_verification_tier(
    screenshot: Screenshot, annotations: list[Annotation]
) -> Literal["single_verified", "agreed", "disputed"]:
    """Determine the verification tier for a screenshot."""
    verifier_ids = screenshot.verified_by_user_ids or []

    if len(verifier_ids) == 0:
        # Not verified - shouldn't happen in this context
        return "single_verified"
    elif len(verifier_ids) == 1:
        return "single_verified"
    else:
        # 2+ verifiers - check for agreement
        # Get annotations from verified users only
        verified_annotations = [a for a in annotations if a.user_id in verifier_ids]
        has_agreement, _ = _compare_annotations(verified_annotations)
        return "agreed" if has_agreement else "disputed"
...
async def get_groups_with_verification_tiers(
    ...
    for group in groups:
        ...
        for screenshot in verified_screenshots:
            tier = _get_verification_tier(screenshot, screenshot.annotations)
...
async def get_screenshots_by_tier(
    ...
    for screenshot in verified_screenshots:
        ...
        screenshot_tier = _get_verification_tier(screenshot, screenshot.annotations)

        if screenshot_tier == tier:
            # Check for differences
            verified_annotations = [a for a in screenshot.annotations if a.user_id in verifier_ids]
            _, differences = _compare_annotations(verified_annotations)
...
async def get_consensus_summary(db: DatabaseSession, current_user: CurrentUser):
    ...
    summary = await consensus_service.get_consensus_summary(db)
    ...
    return ConsensusSummaryResponse(
        ...
        total_disagreements=summary.get("total_disagreements", 0),
        avg_disagreements_per_screenshot=summary.get("avg_disagreements_per_screenshot", 0.0),
    )
...
async def get_consensus_analysis(screenshot_id: int, db: DatabaseSession, current_user: CurrentUser):
    ...
    consensus_service = ConsensusService()
    analysis = await consensus_service.analyze_consensus(db, screenshot_id)
