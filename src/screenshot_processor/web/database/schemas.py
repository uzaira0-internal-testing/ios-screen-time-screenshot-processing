...
class AnnotationBase(BaseModel):
    ...
    def validate_hourly_values(cls, v: dict[str, int | float]) -> dict[str, int | float]:
        """Validate hourly values have valid hour keys and reasonable minute values."""
        for hour_key, minutes in v.items():
            # Check hour key is valid (0-23)
            try:
                hour = int(hour_key)
                if not 0 <= hour <= 23:
            ...
            # Check minutes value is reasonable (0-60, allowing fractional)
            if not isinstance(minutes, (int, float)):
                raise ValueError(f"Minutes value for hour {hour_key} must be a number")
            if minutes < 0:
                raise ValueError(f"Minutes value for hour {hour_key} cannot be negative")
            if minutes > 60:
                raise ValueError(f"Minutes value for hour {hour_key} cannot exceed 60")
