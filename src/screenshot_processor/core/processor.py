...
from .ocr import find_screenshot_title
...
class ScreenshotProcessor:
    ...
    def _create_ocr_engine_from_config(self) -> IOCREngine:
        ...
        if ocr_config.auto_select:
            ...
            engine_kwargs = {}
            if ocr_config.engine_type == "hunyuan":
                engine_kwargs = {
                    "base_url": ocr_config.hunyuan_url,
                    "timeout": ocr_config.hunyuan_timeout,
                    "max_retries": ocr_config.hunyuan_max_retries,
    ...
    def _parse_time_to_minutes(self, total_str: str, calculated_total: float | None = None) -> float:
        ...
        if hours_match:
            with contextlib.suppress(ValueError):
                total_minutes += float(hours_match.group(1)) * 60.0
        ...
        if calculated_total is not None and abs(total_minutes - calculated_total) > 1:
            ...
            if corrected_str != total_str:
                corrected_minutes = 0.0

                hours_match = re.search(r"(\d+)h", corrected_str.lower())
                minutes_match = re.search(r"(\d+)m", corrected_str.lower())

                if hours_match:
                    with contextlib.suppress(ValueError):
                        corrected_minutes += float(hours_match.group(1)) * 60.0

                if minutes_match:
                    with contextlib.suppress(ValueError):
                        corrected_minutes += float(minutes_match.group(1))
