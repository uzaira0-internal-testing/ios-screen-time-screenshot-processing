...
def ocr_results_to_dict(results: list[OCRResult]) -> dict:
    """Convert list of OCRResult objects to pytesseract-style dict.

    This allows HybridOCREngine to be used with existing anchor detection
    functions that expect the pytesseract dict format.

    Args:
        results: List of OCRResult from HybridOCREngine
    ...
    for result in results:
        if result.bbox is not None:
            x, y, w, h = result.bbox
            data["level"].append(5)  # Word level in pytesseract
...
def perform_ocr(
    ...
    if use_hybrid and use_paddleocr_for_grid:
        try:
            ...
            d_left = ocr_results_to_dict(results_left)
            d_right = ocr_results_to_dict(results_right)
