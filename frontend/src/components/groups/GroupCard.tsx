import React from "react";
import { Link } from "react-router";
import type { Group } from "@/core/models";

interface GroupCardProps {
  group: Group;
}

export const GroupCard: React.FC<GroupCardProps> = ({ group }) => {
  const processedPercent =
    group.screenshot_count > 0
      ? Math.round(
          ((group.processing_completed + group.processing_skipped) /
            group.screenshot_count) *
            100,
        )
      : 0;

  return (
    <Link
      to={`/annotate?group=${encodeURIComponent(group.id)}`}
      className="block bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-md transition-all focus-ring"
    >
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">
          {group.name}
        </h3>
        <span className="text-xs text-slate-500 whitespace-nowrap ml-2">
          {new Date(group.created_at).toLocaleDateString()}
        </span>
      </div>

      {/* Total count */}
      <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-100 dark:border-slate-700">
        <span className="text-sm text-slate-600 dark:text-slate-400">Total Screenshots</span>
        <span className="text-lg font-bold text-slate-900 dark:text-slate-100">
          {group.screenshot_count}
        </span>
      </div>

      {/* Processing status grid */}
      <div className="grid grid-cols-2 gap-2 text-center mb-3">
        <div className="bg-primary-50 dark:bg-primary-900/20 rounded p-2">
          <div className="text-lg font-bold text-primary-600">
            {group.processing_pending}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Pending</div>
        </div>
        <div className="bg-green-50 dark:bg-green-900/20 rounded p-2">
          <div className="text-lg font-bold text-green-600">
            {group.processing_completed}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Preprocessed</div>
        </div>
        <div className="bg-red-50 dark:bg-red-900/20 rounded p-2">
          <div className="text-lg font-bold text-red-600">
            {group.processing_failed}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Failed</div>
        </div>
        <div className="bg-slate-50 dark:bg-slate-700/50 rounded p-2">
          <div className="text-lg font-bold text-slate-600">
            {group.processing_skipped}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Skipped</div>
        </div>
      </div>

      {/* Progress bar */}
      {group.screenshot_count > 0 && (
        <div>
          <div className="w-full bg-slate-200 dark:bg-slate-600 rounded-full h-2 overflow-hidden">
            <div className="h-2 flex">
              <div
                className="bg-green-500 transition-all"
                style={{
                  width: `${(group.processing_completed / group.screenshot_count) * 100}%`,
                }}
              />
              <div
                className="bg-slate-400 transition-all"
                style={{
                  width: `${(group.processing_skipped / group.screenshot_count) * 100}%`,
                }}
              />
              <div
                className="bg-red-500 transition-all"
                style={{
                  width: `${(group.processing_failed / group.screenshot_count) * 100}%`,
                }}
              />
            </div>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 text-right">
            {processedPercent}% processed
          </p>
        </div>
      )}
    </Link>
  );
};
