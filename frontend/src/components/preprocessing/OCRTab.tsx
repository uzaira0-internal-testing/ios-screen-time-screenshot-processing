...
function parseOcrTotalMinutes(ocrTotal: string | null | undefined): number | null {
  if (!ocrTotal) return null;
  let minutes = 0;
  const hMatch = ocrTotal.match(/(\d+)\s*h/);
  const mMatch = ocrTotal.match(/(\d+)\s*m/);
...
function OCRTabInner() {
  ...
  const renderResultColumns = (s: Screenshot, event: PreprocessingEventData | null) => {
    ...
    const hasMismatch = ocrMinutes != null && barTotal != null && ocrMinutes > 0 && (
      Math.abs(barTotal - ocrMinutes) > Math.max(ocrMinutes * 0.1, 5)
    );
