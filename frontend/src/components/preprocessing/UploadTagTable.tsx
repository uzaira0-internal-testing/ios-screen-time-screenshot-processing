    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 bg-slate-50 rounded-lg border">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <label className="text-sm font-medium text-slate-700 whitespace-nowrap">Participant ID Regex:</label>
          <input
            type="text"
            value={regexInput}
            ...
            className={`text-sm border rounded-md px-3 py-1.5 flex-1 min-w-[200px] font-mono ${
              regexError ? "border-red-400 bg-red-50" : "border-slate-300"
            }`}
              ...
              <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="px-3 py-1.5">
                  {thumbnails[i] ? (
                    ...
                    <div className="w-8 h-11 bg-slate-200 rounded" />
                  )}
                </td>
                <td className="px-3 py-1.5 text-xs text-slate-600 max-w-xs truncate" title={item.original_filepath}>
                  {item.filename}
                </td>
                <td className="px-3 py-1.5">
                  <input
