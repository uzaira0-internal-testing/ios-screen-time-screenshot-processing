...
use std::{env, fs, io::{self, BufRead}, time::Instant};
use serde_json::json;
...
fn main() {
    ...
    let paths: Vec<String> = if stdin_mode {
        io::stdin().lock().lines().filter_map(|l| l.ok()).filter(|l| !l.is_empty()).collect()
    } else {
        let dir = args.iter().find(|a| !a.starts_with('-') && a.as_str() != args[0].as_str()).map(|s| s.as_str()).unwrap_or("/tmp/test-images");
        fs::read_dir(dir)
    ...
    if !jsonl {
        ...
        println!("{:<50} {:>10} {:>10} {:>8} {:>8}", "Image", "Method", "Time(ms)", "Total", "Score");
        println!("{}", "-".repeat(90));
    }
    ...
    for path in &paths {
        ...
        let img = match image::open(path) {
            ...
            Err(e) => {
                if jsonl {
                    println!("{}", json!({"path": path, "ok": false, "err": format!("{e}")}));
                } else {
        ...
        match grid_result {
            Ok(ref r) if r.success => {
                ...
                if jsonl {
                    let hourly_str: String = row[..24].iter().map(|v| format!("{v:.1}")).collect::<Vec<_>>().join(",");
                    println!("{}", json!({
                        "path": path,
                        "ok": true,
                    ...
                    println!("{name:<50} {res:<10} {elapsed:>7}ms {total:>8.1} {score:>7.2}",
                        res = format!("{w}x{h}"));
            ...
            Ok(ref r) => {
                ...
                if jsonl {
                    println!("{}", json!({"path": path, "ok": false, "err": r.error.as_deref().unwrap_or("unknown"), "ms": elapsed}));
                } else {
                    println!("{name:<50} {res:<10} {elapsed:>7}ms GRID_FAIL: {}",
                        r.error.as_deref().unwrap_or("unknown"), res = format!("{w}x{h}"));
                }
            }
            Err(e) => {
                let elapsed = start.elapsed().as_millis();
                if jsonl {
                    println!("{}", json!({"path": path, "ok": false, "err": format!("{e}"), "ms": elapsed}));
                } else {
                    println!("{name:<50} ERROR: {e}");
                }
            }
    ...
    if !jsonl && processed > 0 {
        println!("{}", "-".repeat(90));
        println!("Processed {processed}/{} images. Total: {total_time_ms}ms, Avg: {}ms/image",
            paths.len(), total_time_ms / processed as u64);
    }
}
