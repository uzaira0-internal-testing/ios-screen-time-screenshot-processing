...
use super::bar_extraction::slice_image_fast_total;
...
/// Extract bar values for the given bounds and return their sum in minutes.
/// Extract bar total for the given bounds using fast 1x path (no 4x upscale).
///
/// The optimizer calls this ~1300 times per image, so skipping the 4x scale
/// is critical — it's ~16x faster per call. Sub-pixel precision doesn't
/// matter here since we only compare totals to find the best shift.
fn extract_bar_total(img: &RgbImage, bounds: &GridBounds, image_type: ImageType) -> Option<f64> {
    ...
    let total = if image_type == ImageType::Battery {
        ...
        // Dark mode fallback: retry with inverted battery color
        if t == 0.0 {
        ...
        slice_image_fast_total(img, roi_x as u32, roi_y as u32, roi_w as u32, roi_h as u32)
    };
...
pub fn optimize_boundaries(
    ...
    let target_minutes = match parse_ocr_total(ocr_total) {
        ...
        None => {
            ...
            let bar_total = extract_bar_total(img, initial_bounds, image_type)
    ...
    let initial_bar_total = extract_bar_total(img, initial_bounds, image_type)
        .unwrap_or(0.0) as i32;
    ...
    while sx <= max_shift {
        ...
        while sy <= max_shift {
            ...
            while sw <= max_shift {
                ...
                let bar_total = match extract_bar_total(img, &test_bounds, image_type) {
                    Some(t) => t as i32,
                    None => { sw += 2; continue; }
                };
